...
var currencyDictionary = function(rubles) {
  var text = rubles(44.20);
  text.should.equal('сорок четыре рубля 20 копеек');

  text = rubles(44.20, [['рубль', 'рубля', 'рублей'], ['копейка', 'копейки', 'копеек']]);
  text.should.equal('сорок четыре рубля 20 копеек');

  text = rubles(44.20,[['доллар', 'доллара', 'долларов'], ['цент', 'цента', 'центов']]);
  text.should.equal('сорок четыре доллара 20 центов');
};
